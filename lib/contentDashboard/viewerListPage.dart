...
class ViewerListPage extends StatefulWidget {
  const ViewerListPage({Key key}) : super(key: key);

  @override
  State<ViewerListPage> createState() => _ViewerListPageState();
}

class _ViewerListPageState extends State<ViewerListPage> {
  StreamController streamViewersList = StreamController.broadcast();
  StreamController streamPendingViewersList = StreamController.broadcast();
  bool centerProgressIndicator = false;
  bool spin = false;
  List pendingViewersData = [];
  final GlobalKey<ScaffoldState> _scaffoldKey = new GlobalKey<ScaffoldState>();

  @override
  void initState() {
    ContentDashboardService().getViewersList().then((val) {
      streamViewersList.add(val);
    });

    ContentDashboardService().getPendingViewersList().then((val) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          InkWell(
              onTap: () {
                requestDialog(context, height * 0.80, width);
              },
              child: Row(
        ...
        child: Stack(
          children: [
            ListView(
              children: [
                StreamBuilder(
                  stream: streamViewersList.stream,
               ...
               // if(pendingViewersData.length > 0)
                Text(
                  "Pending Viewers",
                ...
                StreamBuilder(
                  stream: streamPendingViewersList.stream,
                  builder: (context, snapshot) {
                    if (snapshot.hasData) {
                      return SingleChildScrollView(
                        child: ListView.builder(
